...
layui.define(['jquery','layer','laytpl','element'],function(exports){
      ...
      if(name !== undefined){
           ...
           for(var i=0;i<scripts.length;i++){
               if(name == scripts[i].src.substring(scripts[i].src.lastIndexOf('/')+1)){
                  ...
                  jsPath['url'] = urlStr.substring(urlStr.indexOf('/'),urlStr.lastIndexOf('/layui/')+1)+'js/plus/';
                  jsPath['index'] = i;
               }
           }
           jsPath['obj'] = scripts;
    ...
    //larryms构造器
    larryms = function(){
        this.version = 'LarryMS 2.07 Beta1',
    ...
    // 退出系统注销登录
    larryms.prototype.logOut = function(ok,no){
       this.confirm('确定退出系统吗！',ok,no=goback);   
    };
