   ...
   var $ = layui.jquery,
	...
	LarryTab.prototype.init = function() {
		...
		if (typeof(_config.elem) !== 'string' && typeof(_config.elem) !== 'object') {
			common.cmsError('系统 error: elem参数未定义或设置出错，具体设置格式请参考文档API.', 'elem参数错误');
		}
		var $container;
		...
		if ($container.length === 0) {
			common.cmsError('系统 error: 找不到elem参数配置的容器', '容器参数设置错误');
		}
		var filter = $container.attr('lay-filter');
		if (filter === undefined || filter === '') {
			common.cmsError('系统 error: 请为elem容器设置一个lay-filter过滤器', 'lay-filter过滤器设置错误');
		}
	...
	LarryTab.prototype.ajaxPage = 0;

	/**
	 * 使指定tabid获取焦点
	 */
	LarryTab.prototype.setCurrentTabId = function(id) {
		var that = this;
		var _config = that.config;
		
		$(_config.elem).find('ul.layui-tab-title').children('li').each(function(a,b){
		...
		switch(eventsName){
			case 'closeCurrent': //关闭当前
			    if (currentTabID > 0) {
			    	...
			    	common.cmsError('系统 提示您：默认首页不能关闭的哦！','关闭失败提示');
			    }
				break;
			case 'closeOther': //关闭其他
			    if(ELEM.titleBox.children('li').length>2){
			    	...
			    	common.cmsError('系统 提示您：其他选项卡【默认首页】不能关闭！', '关闭失败提示');
			    }else{
			    	common.cmsError('系统 提示您：当前无其他可关闭选项卡！', '关闭失败提示');
			    }
				break;
			case 'closeAll': //全部关闭
    ...
    LarryTab.prototype.on = function(events,callback){
		...
		if (typeof(events) !== 'string') {
			common.cmsError('系统 Nav error:事件名配置出错，请参考API文档.', '事件名配置出错')
		}
		...
		if (eventName === 'click'){
		 	if (ELEM.tabCtrlBox.attr('lay-filter') == 'larryOperate'){
		    	...
		    	common.cmsInfo('系统 Nav error:当前传入的lay-filter是'+filter+';而非larryOperate', '常用操作不匹配');
		    }
		}else{
		 	common.cmsInfo('系统 Nav error:当前传入的事件名是'+eventName+';非点击事件，其他功能稍后续！', '事件名不匹配提示');
		}
    };
